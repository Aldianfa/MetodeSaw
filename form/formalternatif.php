...
if (isset($_POST['simpan'])) {
    ...
    if ($_GET['hal'] == "edit") {
        ...
        //data di edit
        $edit = $koneksi->query("UPDATE tb_alternatif SET id_alternatif = '$_POST[idalternatif]', nm_alternatif = '$_POST[nmalternatif]', 
                                WHERE id_alternatif = '$_GET[id]'");


        if ($edit == true) {
            echo "<script>alert('Edit SUKSES');
                window.location.href=('formalternatif.php');</script>";
        } else {
            var_dump($edit);
        ...
        //data disimpan baru
        $insert = $koneksi->query("INSERT INTO `tb_alternatif` (id_alternatif, nm_alternatif) 
                                        VALUES ('$_POST[idalternatif]', '$_POST[nmalternatif]');");
...
if (isset($_GET['hal'])) {
    //Pengujian jika edit
    if ($_GET['hal'] == "edit") {
        $tampil = mysqli_query($koneksi, "SELECT * FROM tb_alternatif WHERE id_alternatif = '$_GET[id]' ");
        $data = mysqli_fetch_array($tampil);
        if ($data) {
            $idalternatif = $data['id_alternatif'];
            $nmalternatif = $data['nm_alternatif'];
        }
    } else if ($_GET['hal'] == "hapus") {
        $hapus = mysqli_query($koneksi, "DELETE FROM tb_alternatif WHERE id_alternatif = '$_GET[id]'");
        ...
        <div class="card mt-3">
            ...
            <div class="card-body">
                <form action="" method="POST">
                    <div class="row">
                        <div class="mb-3">
                            <label for="exampleInputEmail1" class="form-label">Id Alternatif</label>
                            <input type="text" class="form-control" name="idalternatif" required="" placeholder="idalternatif" value="<?= @$idalternatif ?>">

                            <label for="exampleInputEmail1" class="form-label mt-4">Nama Alternatif</label>
                            <input type="text" class="form-control" name="nmalternatif" required="" placeholder="nama alternatif" value="<?= @$nmalternatif ?>">
                        </div>
                    </div>
                    <div class="row ">
                ...
                <table class="table table-bordered table-striped">
                    ...
                    ?>
                        <tr>
                            ...
                            <td>
                                <a href="formalternatif.php?hal=edit&id=<?= $c['id_alternatif'] ?>" class="btn btn-warning"> Edit </a>
                                <a href="formalternatif.php?hal=hapus&id=<?= $c['id_alternatif'] ?>" onclick="return confirm('Yakin Tabel Ini Dihapus?')" class="btn btn-Danger"> Drop </a>
                            </td>
                        </tr>
                    <?php
                    }
