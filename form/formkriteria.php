...
if(isset($_POST['simpan'])){
    ...
    //data di edit
        $edit = $koneksi -> query("UPDATE tb_kriteria SET id_kriteria = '$_POST[idkriteria]', nm_kriteria = '$_POST[nmkriteria]', 
                                tipe = '$_POST[tipe]' WHERE id_kriteria = '$_GET[id]'");

        if ($edit == true){
            echo "<script>alert('Edit SUKSES');
                window.location.href=('formkriteria.php');</script>";
        }else{
    ...
    //data disimpan baru
        $insert = $koneksi -> query("INSERT INTO `tb_kriteria` (id_kriteria, nm_kriteria, tipe) 
                                        VALUES ('$_POST[idkriteria]', '$_POST[nmkriteria]', '$_POST[tipe]');");
...
if(isset($_GET['hal']))
{
    ...
    if($_GET['hal'] == "edit")
    {
        $tampil = mysqli_query($koneksi, "SELECT * FROM tb_kriteria WHERE id_kriteria='$_GET[id]' ");
        $data = mysqli_fetch_array($tampil);
        if($data)
        {
            $idkriteria = $data['id_kriteria'];
            $nmkriteria = $data['nm_kriteria'];
            $tipe = $data['tipe'];
        }
    }else if($_GET['hal'] == "hapus"){
        $hapus = mysqli_query($koneksi, "DELETE FROM tb_kriteria WHERE kd_kriteria = '$_GET[id]'");
                ...
                <form action="" method="POST">
                    <div class="row">
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control" name="idkriteria" required="" placeholder="idkriteria" value="<?=@$idkriteria?>" >

                            <label for="exampleInputEmail1" class="form-label mt-4">Nama Kriteria</label>
                            <input type="text" class="form-control" name="nmkriteria" required="" placeholder="nmkriteria" value="<?=@$nmkriteria?>">


                            <label for="exampleInputEmail1" class="form-label mt-4">Tipe</label>
                            <input type="text" class="form-control" name="tipe" required="" placeholder="tipe" value="<?=@$tipe?>">
                        </div>
                    </div>
                    <div class="row ">
                ...
                <table class="table table-bordered table-striped">
                    ...
                    ?>
                        <tr>
                            ...
                            <td>
                                <a href="formkriteria.php?hal=edit&id=<?=$c['id_kriteria']?>" class="btn btn-warning"> Edit </a>
                                <a href="formkriteria.php?hal=hapus&id=<?=$c['id_kriteria']?>" 
